...
{
    /// <summary>
    /// Extended Cryptography backend interface. This interfaces provides predicate methods to
    /// verify in multiple scheme.
    /// </summary>
    /// <typeparam name="T">A <see cref="HashAlgorithm"/> which corresponds to a digest.</typeparam>
    /// <seealso cref="HashAlgorithm"/>
    public interface IExtendedCryptoBackend<T>
        where T : HashAlgorithm
    {
        void Validate(ICryptoType privateKey, Action<ICryptoType> predicate);
        ...
        /// <paramref name="privateKey"/>.
        /// </returns>
        byte[] Sign(HashDigest<T> messageHash, ICryptoType privateKey);
        ...
        /// <param name="predicate">A predication method for verification.</param>
