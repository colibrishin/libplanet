...
namespace Libplanet.Tests.Crypto
{
    public class BlsPrivateKeyTest
    {
        ...
        {
            var privateKey = new BlsPrivateKey(PrivKey1);
            IECPrivateKey ecPrivateKey = privateKey;
            AssertBytesEqual(PrivKey1, ecPrivateKey.KeyBytes.ToArray());
        }
        ...
        [Fact]
        public void IECPrivateKeySignTest()
        {
            IECPrivateKey ecPrivateKey = new BlsPrivateKey(PrivKey1);
            var pubKey = ecPrivateKey.PublicKey;
            var wrongPubKey = new BlsPrivateKey().PublicKey;

            AssertBytesEqual(ecPrivateKey.Sign(_payload), ecPrivateKey.Sign(_payload));
            Assert.True(pubKey.Verify(_payload, ecPrivateKey.Sign(_payload)));
            Assert.False(pubKey.Verify(_payload.Skip(1).ToArray(), ecPrivateKey.Sign(_payload)));
            Assert.Throws<CryptographicException>(
                () => pubKey.Verify(
                    _payload, ecPrivateKey.Sign(_payload).Skip(1).ToArray()));
            Assert.False(wrongPubKey.Verify(_payload, ecPrivateKey.Sign(_payload)));
            Assert.True(pubKey.Verify(_payload, ecPrivateKey.Sign(_payload)));
        }
