...
namespace Libplanet.Crypto.Common.Tests
{
    public class PrivateKeyTest
    {
        ...
        public void SignTest()
        {
            ...
            var imPayload = _payload.ToImmutableArray();
            Assert.False(pubKey.Verify(_payload.Skip(1).ToArray(), pk.Sign(imPayload).ToArray()));
            Assert.False(pubKey.Verify(_payload, pk.Sign(imPayload).Skip(1).ToArray()));
            Assert.False(wrongPubKey.Verify(_payload, pk.Sign(imPayload).ToArray()));
        }

        [Fact]
        public void IPrivateKeySignTest()
        {
            IPrivateKey pk = new PrivateKey(_key1);
            var pubKey = pk.PublicKey;
            var wrongPubKey = new PrivateKey().PublicKey;
